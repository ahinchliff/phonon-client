...
func postPhonons(c *ishell.Context) {
	...
	c.Println(`Should the packet be private? Y/N`)
	privateInput := c.ReadLine()
	isPrivate := privateInput == "Y" || privateInput == "y"

	transferPhononPackets, err := activeCard.PostPhonons(isPrivate, receivingCardPubKeyXbytes, nonce, keyIndices)
	if err != nil {
